 ...
 * returns:     the calculated checksum.
 *--------------------------------------------------------------------*/
unsigned short i3dmGX1_Checksum( unsigned char* pBytes, int count)
{
	unsigned short i3_checksum = 0x000E;
	unsigned short temp;
	int i = 0;
	//int Vcount = count/2;

	//i3_checksum+=pBytes[0];
	//count = 4;

	for(i = 1; i < count; ++i)
	{
		//temp=convert2ushort(pBytes[i*2]);
		i3_checksum+=((pBytes[i]*256)+ pBytes[++i]);
		//i3_checksum+=temp;
		//fprintf("count is %i", i);
	}

	return i3_checksum;
}
/*unsigned short i3dmGX1_Checksum(const unsigned char* pBytes, int count)
{
 ...
 * Converts bytes(In Little Endian Format) to Float.
 *
 * parameters:  pBytes : received buffer containing pointer to 4 bytes
 *
 * returns:     a float value.
 *--------------------------------------------------------------------*/
float Little_Endian_Float(unsigned char* pBytes)
{
   float f=0; //val;
...
/* **************************************************************
*   ReadCharNoReturn:
*   checks for a termination code and exits continious mode
****************************************************************/
